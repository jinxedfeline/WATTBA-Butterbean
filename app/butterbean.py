...
async def checkApprovedUsers(user: str) -> bool:
    return hasPermission(user, 'manage_memes')
...
# TODO: refactor this away from "pronoun" to the more general "role"
...
class PronounPicker(discord.ui.Select):
    def __init__(self, interaction: discord.Interaction):
        ...
        valid_pronouns = list(filter(lambda r: r.name in assignableRoles, interaction.guild.roles))
        ...
        #   to remove all tags
        super().__init__(placeholder='Choose which roles you\'d like to have', min_values=0, max_values=len(options), options=options)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f'Your roles are now {", ".join(self.values) if len(self.values) > 0 else "(none)"}', ephemeral=True)


# add the slash command to the bot's command tree
@client.tree.command(description='Get a menu to pick your pronouns and other roles from')
async def pickpronoun(interaction: discord.Interaction):
    ...
    await interaction.response.send_message('Please choose any number of roles:', view=view, ephemeral=True)
